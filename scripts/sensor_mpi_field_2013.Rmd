---
title: "Sensor Data MPI Field 2013"
...
# load workspace
#load("sensor_mpi_field_2013.RData")
...
### read data table
```{r read data table}
raw <- read.table("data/sensors_logger1_2013_05_17-2013_08_14.txt", header=T, fill=T)

# meta information
meta <- read.table("data/sensors_metadata_logger_sensor_channel.txt", header=T)
meta_sub <- droplevels( subset(meta, meta$Study_ID==62326))
...
# subset after 2013-05-17 and before 2013-08-14
raw_sub <- subset(raw, Date > as.Date("2013-05-17") & Date < as.Date("2013-08-14") )

colnames(raw_sub)

# only the first 39 moisture columns (every second column from 2 to 78)
moisture <- raw_sub[, c(seq(2, 78, by=2))]
...
pdf("figures/sensors_moisture_mpi_field_2013.pdf", width=12, height=8)
...
### save workspace
```{r save workspace}
save.image("sensor_mpi_field_2013.RData")
```
