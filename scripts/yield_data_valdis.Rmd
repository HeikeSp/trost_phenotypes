...
lines_2014 <- c(common_lines_2014, "AR21")
length(lines_2014)
```
...
yield_data_correct_2014 <- droplevels(subset(yield_data_correct, yield_data_correct$culture %in% cultures_2014))
...
# 2014
mpi_fgh_2014_control_median <- median(mpi_fgh_2014_control_mean_per_line$mean, na.rm = T)
# 117.20
mpi_feld_2014_control_median <- median(mpi_feld_2014_control_mean_per_line$mean, na.rm = T)
# 261.99
jki_feld_2014_control_median <- median(jki_feld_2014_control_mean_per_line$mean, na.rm = T)
# 324.64
jki_shelter_2014_control_median <- median(jki_shelter_2014_control_mean_per_line$mean, na.rm = T)
# 56.16

# 2015
mpi_fgh_2015_control_median <- median(mpi_fgh_2015_control_mean_per_line$mean)
# 272.73
mpi_feld_2015_control_median <- median(mpi_feld_2015_control_mean_per_line$mean)
# 221.56
jki_feld_2015_control_median <- median(jki_feld_2015_control_mean_per_line$mean)
# 242.85
jki_shelter_2015_control_median <- median(jki_shelter_2015_control_mean_per_line$mean)
# 71.91
dethlingen_2015_control_median <- median(dethlingen_2015_control_mean_per_line$mean)
# 216.81
```
...
```{r calculate mean of starch yield per line and experiment}
# 2014
# calculate mean of starch yield per line and experiment
...
all_SI_2014 <- unlist(lapply(dflist_2014, func_calc_stress_index_valdis))
...
all_SI_2015 <- unlist(lapply(dflist_2015, func_calc_stress_index_valdis))
```
...
```{r calculate RelSY}
# 2014
mpi_fgh_2014_relSY <- func_calc_relSY_valdis(mpi_fgh_2014, plant_lines = levels(yield_data_final_2014$alias))
mpi_feld_2014_relSY <- func_calc_relSY_valdis(mpi_feld_2014, plant_lines = levels(yield_data_final_2014$alias))
jki_feld_2014_relSY <- func_calc_relSY_valdis(jki_feld_2014, plant_lines = levels(yield_data_final_2014$alias))
jki_shelter_2014_relSY <- func_calc_relSY_valdis(jki_shelter_2014, plant_lines = levels(yield_data_final_2014$alias))

# 2015
mpi_fgh_2015_relSY <- func_calc_relSY_valdis(mpi_fgh_2015, plant_lines = levels(yield_data_final_2015$alias))
mpi_feld_2015_relSY <- func_calc_relSY_valdis(mpi_feld_2015, plant_lines = levels(yield_data_final_2015$alias))
jki_feld_2015_relSY <- func_calc_relSY_valdis(jki_feld_2015, plant_lines = levels(yield_data_final_2015$alias))
jki_shelter_2015_relSY <- func_calc_relSY_valdis(jki_shelter_2015, plant_lines = levels(yield_data_final_2015$alias))
dethlingen_2015_relSY <- func_calc_relSY_valdis(dethlingen_2015, plant_lines = levels(yield_data_final_2015$alias))
...
mpi_fgh_2014_drym <- func_calc_drym(mpi_fgh_2014_relSY, mpi_fgh_2014_relSY_median, 
                                    cultivar_names = levels(yield_data_final_2014$alias))
...
mpi_fgh_2015_drym <- func_calc_drym(mpi_fgh_2015_relSY, mpi_fgh_2015_relSY_median, 
                                    cultivar_names = levels(yield_data_final_2015$alias))
...
drym_experiments_2014_melt$culture <- rep( sort(unique(yield_data_final_2014$culture)), 237)
...
pdf("figures/valdis_boxplot_drym_per_line_sp.pdf", width = 9, height = 5)
par(mfrow = c(1,3))
...
setdiff(levels(drym_experiments_2014_melt$alias), levels(yield_data_final_2014_control_norm_sy_mean$alias))

drym_experiments_2014_melt <- drym_experiments_2014_melt[-which(drym_experiments_2014_melt$alias=="AR233"),]
...
head(mpi_feld_2015_relSY)
# median per line
head(mpi_feld_2015_relSY_median)

mpi_feld_2015_ssi <- list()
  for (plant_line in levels(yield_data_final_2015$alias) ){
    mpi_feld_2015_ssi[[plant_line]] <- (1 - unique(mpi_feld_2015_relSY_median[[plant_line]]) ) / mpi_feld_2015_si
  }

plot(unlist(mpi_feld_2015_ssi), unlist(mpi_feld_2015_relSY_median))
plot(unlist(mpi_feld_2015_ssi), unlist(mpi_feld_2015_drym_median))

# negative correlation between drym and ssi --> higher ssi means more sensitive

test <- func_calc_ssi(mpi_feld_2015, plant_lines = levels(yield_data_final_2015$alias), mpi_feld_2015_si)
test <- func_calc_ssi(mpi_feld_2015, plant_lines = levels(yield_data_final_2015$alias), mpi_feld_2015_si)
